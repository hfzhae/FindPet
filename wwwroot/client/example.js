...
$(function() {
    ...
    getData('version', function(data) {
        if (!storage.getItem("cachever")) {
            storage.cachever = data.v;
        } else {
            if (storage.cachever != data.v) {
                storage.removeItem("menuArr");
                storage.cachever = data.v;
            }
        }

        if (!storage.getItem("menuArr")) {
            var l = $.toast('正在读取...', 'loading', 0, null, 0);
